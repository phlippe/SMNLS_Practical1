...
def create_dataset_from_file(input_file):
	data_list = list()
	with open(input_file, "r") as f:
		for line in f:
			if not "#SEP#" in line:
				print("[!] ERROR: Every line in the input file must contain the string \"#SEP#\" in between the premise and the hypothesis.")
...
def run_inference(model, input_file, output_file, batch_size=64):
	infer_dataset = create_dataset_from_file(input_file)
...
if __name__ == '__main__':
	...
	run_inference(model, args.input_file, args.output_file)
